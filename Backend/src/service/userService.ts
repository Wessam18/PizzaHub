...
const generateJWT = (data: { name: string; email: string; phoneNumber: string }): string => {
    return jwt.sign(data, process.env.JWT_SECRET || "default_secret_key", { expiresIn: "24h" });
};

export const register = async ({ name, email, phoneNumber, password }: { name: string; email: string; phoneNumber: string; password: string; }) => {
...
const generatedJWT = (data: { name: string; email: string; phoneNumber: string }): string => {
